...
def plot_tracks(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):
...
def plot_trajectory(image, tlwhs, track_ids):
    ...
    for one_tlwhs, track_id in zip(tlwhs, track_ids):
        ...
        for tlwh in one_tlwhs:
            ...
            cv2.circle(image, (int(x1 + 0.5 * w), int(y1 + h)),
                       2, color, thickness=2)
...
def plot_detections(image, tlbrs, scores=None, color=(255, 255, 255), ids=None):
    im = np.copy(image)
